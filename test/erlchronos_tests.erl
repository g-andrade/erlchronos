...
basic_tick_works() ->
    ...
    NapTime = 1000,
    {ok, Pid} = erlchronos_ticked_gen_serv:start_link([{ticks, Ticks}] ++ SpawnOpts, NapTime),
    timer:sleep(NapTime),
...
gen_server_wrapping_works() ->
    SpawnOpts = [{spawn_opts, [{priority, high}]}],
    {ok, TickedPid} = erlchronos_ticked_gen_serv:start_link([{ticks, [{tick, 10}]}] ++ SpawnOpts, 0),
...
moderate_message_flood_is_tolerable() ->
    ...
    FloodDuration = 2000,
    {ok, Pid} = erlchronos_ticked_gen_serv:start_link([{ticks, Ticks}] ++ SpawnOpts, FloodDuration),
